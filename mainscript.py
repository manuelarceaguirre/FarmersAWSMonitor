...
class EvidentlyAssistant:
    ...
    def save_powerbi_format(self, results: Dict, output_path: str):
        try:
            feature_data = []
            ...
            if feature_data:
                ...
                df['Normalized_Importance'] = df[importance_col].rank(pct=True)
                
                # Normalize drift using percentile ranking
                df['Normalized_Drift'] = df['Drift_Score'].rank(pct=True)
                
                # Calculate Priority Score
                df['Priority_Score'] = (df['Normalized_Importance'] + df['Normalized_Drift']) / 2
                ...
                # Get the month from current data for filename
                current_month = None
                if 'Time' in self.current_data.columns:
                    current_month = self.current_data['Time'].iloc[0]
                else:
                ...
                final_output_path = os.path.join(output_dir, f"{base_name}_{current_month}{ext}")
                
                # Create directory if it doesn't exist
                os.makedirs(output_dir, exist_ok=True)
                
                # Save to CSV with consistent formatting
                df.to_csv(final_output_path, index=False, float_format='%.4f')
    ...
    def calculate_drift_scores(self) -> Dict:
        ...
        for i, column in enumerate(columns_to_analyze, 1):
            ...
            try:
                ...
                if column_config['type'] == 'numerical':
                ...
                if metrics_result and 'drift_score' in metrics_result:
                    ...
                    drift_results[column] = {
                        ...
                        'type': str(column_config['type']),
                        'tests': [str(test) for test in column_config['tests']]
                    }
...
def main():
    ...
    assistant.save_powerbi_format(results, output_path)
